sumstats.mod <- function(files,ref,trait.names=NULL,se.logit,OLS=NULL,linprob=NULL,prop=NULL,N=NULL,info.filter = .6,maf.filter=0.01,keep.indel=FALSE,parallel=FALSE,cores=NULL){
  ...
  if(parallel == FALSE){
    ...
    #mod change
    ref<-read.table(ref,header=T, quote="\"",fill=T,na.string=c(".",NA,"NA",""))
